...
const makeSynths = (count, synthSelect) => {
  ...
  for (let i = 0; i < count; i++) {
   ...
   /*  let synth = new Tone.Synth({
    ...
    switch (synthSelect) {
      ...
      break;
      case 'square8':
        synth = new Tone.Synth({oscillator: {type: "square8"}});
      break;
    } 
...
let synths = makeSynths(6, synthSelect.value);
...
bpm.addEventListener("input", () => {
  ...
  bpm.nextElementSibling.innerText = bpm.value;
});
...
/*  change synth preset */
synthSelect.addEventListener("input", () => {
  synths = makeSynths(6, synthSelect.value);
}); 
